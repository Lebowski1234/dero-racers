...
Before playign the game, please read the following sections for important information:
...
Please read the next two sections before playing the game (Playing the Game, and Rules).
...
* When new cars are 'minted' by the contract owner (for example when the game was released) there is a purchase limit of one car every 50 blocks for each player. So you must wait 50 blocks before buying another car. 
